	...
	class Tabbed_Settings extends Extendible_Tabbed_Settings {
		...
		public function field_page_select_list_option( array $args  ) {
			...
			wp_dropdown_pages( array( 
									...
									'show_option_none' => _x( "- None -", 'text for no page selected', 'small-groups' ), 
									'option_none_value' => '0', 
		...
		public function field_plugin_checkbox_option( array $args  ) {
			...
			if ( ! file_exists( $plugin_main_file ) ) {
				echo esc_html__( 'Enable to prompt installation and force active.', 'small-groups' ) . ' ( ';
				if ( $value ) {
                                    echo '  <a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '">' .  _x( 'Install', 'Install the Plugin', 'small-groups' ) . " </a> | " ;
                                }
				
			} elseif ( is_plugin_active( $option['slug'] . '/' . $option['slug'] . '.php' ) &&  ! is_plugin_active_for_network( $option['slug'] . '/' . $option['slug'] . '.php' ) ) {
				echo esc_html__( 'Force Active', 'small-groups' ) . ' ( ';
				if ( ! $value ) { 
                                    echo '<a href="plugins.php?s=' . esc_html( $option['label'] )	 . '">' .  _x( 'Deactivate', 'deactivate the plugin', 'small-groups' ) . "</a> | " ;	
                                }
			} else {
				echo esc_html__( 'Force Active', 'small-groups' ) . ' ( ';
			}
			echo ' <a href="http://wordpress.org/plugins/' . esc_html( $option['slug'] ) . '">' .  esc_html__( "wordpress.org", 'small-groups' ) . " </a> )" ;		
			?></label><?php
			if ( ! empty( $option['desc'] ) ) {
				echo ' <p class="description">' .  $option['desc']  . '</p>';
                        }
